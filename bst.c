...
int get_subtree_size(struct bst_node* node) {
  int size;
  if (node == NULL) {
    return 0;
  } else {
    size = get_subtree_size(node->left) + get_subtree_size(node->right) + 1;
    return size;
  }
}
...
int bst_size(struct bst* bst) {
  if (bst->root == NULL) {
    ...
    return get_subtree_size(bst->root);
  }
}
...
int get_path_sum(int sum, struct bst_node* node) {
  int num = 0;
  int sub_tree_sum;
  if (node == NULL) {
    return 0;
  } else {
    sub_tree_sum = sum - node->val;
    if (sub_tree_sum == 0 && node->left == NULL && node->right == NULL) {
      return 1;
    }
    if (node->left) {
      num = num || get_path_sum(sub_tree_sum, node->left);
    }
    if (node->right) {
      num = num || get_path_sum(sub_tree_sum, node->right);
    }
    return num;
  }
}
...
int bst_path_sum(int sum, struct bst* bst) {
  if (bst->root == NULL) {
    ...
    return get_path_sum(sum, bst->root);
  }
}
...
struct bst_iterator* reverse_stack(struct bst_iterator* iter) {
  struct stack* temp_1 = stack_create();
  struct stack* temp_2 = stack_create();
  while (stack_isempty(iter->s) == 0) {
    stack_push(temp_1, stack_pop(iter->s));
  }
  while (stack_isempty(temp_1) == 0) {
    stack_push(temp_2, stack_pop(temp_1));
  }
  while (stack_isempty(temp_2) == 0) {
    stack_push(iter->s, stack_pop(temp_2));
  }
  stack_free(temp_1);
  stack_free(temp_2);
  return iter;
